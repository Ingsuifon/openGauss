...
Odess::Odess() {
  ...
  std::uniform_int_distribution<uint64_t>::param_type paramA(
      0x0000000000100000, 0x0000000010000000);
  std::uniform_int_distribution<uint64_t>::param_type paramB(
      0x0000000000100000, 0x00000000ffffffff);
...
void Odess::Calculation(uint8_t* buffer, uint64_t length,
  ...
  for (uint64_t i = 0; i < length; i++) {
    ...
    for (int j = 0; j < 12; j++) {
      ...
      if (transResult > maxList[j]) maxList[j] = transResult;
    }
  }
